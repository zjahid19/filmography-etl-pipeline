...
class MovieParser:
    ...
    def parse_movies(self, filmography_url:str) -> list:
        ...
        if content:
            ...
            table = soup.find(name='table', attrs={'class': 'wikitable sortable plainrowheaders'})
            if table:
                movie_links = table.find_all(name='a')
                movies = [self.base_url + link['href'] for link in movie_links if link.get('href', '').startswith('/wiki/')]
