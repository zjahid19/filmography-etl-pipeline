...
class Connection:
    def __init__(self, config_file):
        ...
        logger.info("Table Name - movies_details")
...
class DataFrameLoading:
    ...
    def data_load(self):
        try:
            logger.info(
                f"Number of records in source dataframe : {len(self.dataframe)}"
            )
            ...
            logger.error(
                f"There is an Error while loadin the data into {self.table_name}", e
            )
...
class DataLoading:
    ...
    def loading(self):
        ...
        con = Connection(
            "/workspaces/filmography-etl-pipeline/filmography-etl-pipeline/credentials.json"
        )
        # Testing Connection
        connection_object = con.established_connection()

        dataframe_loading = DataFrameLoading(
            connection_object, self.result_df, "movies_details"
        )
